...
def main_loop():
	...
	while (cv2.waitKey(1) & 0xFF) != 27:  # 27对应esc键的ASCII码
		...
		try:
			...
			# 获取二值图像的尺寸
			whole_h, _ = red_mask.shape[:2]
			# 在蓝色掩码中查找轮廓
			_, contours, _ = cv2.findContours(image=red_mask, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_NONE)
			# 将轮廓转换为列表并按面积降序排列
			...
			for cont in contours:
				...
				if h / w >= 1.5 and h / w <= 3.5 and h / whole_h > 0.050 and h > w and h / whole_h < 0.5:
					#print("h:", h)
					#print("w:", w)
					#print("x:", x)
					#print("y:", y)
			...
			for i in range(len(rects) - 1):
				for j in range(i + 1, len(rects)):
					...
					#print("ratio:", ratio)
					# 添加额外的条件：y坐标差距不能太大
						if value < min_value and y_diff < 90 and 2.15 <= ratio <=2.7:  # 可改
							min_value = value
