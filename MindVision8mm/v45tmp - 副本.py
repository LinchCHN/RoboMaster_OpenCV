...
while True:
    ...
    for cont in contours:
        ...
        # 检查有效矩形的条件    #[:2]代表第1个到第2轮廓（矩形）  x，y代表单个矩形的左上坐标   h / whole_h代表着蓝条的长短
        if  h / w >= 1 and h / w <= 3.5 and h / whole_h > 0.050 and h  > w :#   and h / whole_h < 0.3 :
    ...
    for i in range(len(rects) - 1):
        for j in range(i + 1, len(rects)):
            ...
            # 添加额外的条件：y坐标差距不能太大
            if value < min_value and y_diff < 400:#可改
                min_value = value
